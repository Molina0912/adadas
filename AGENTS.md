...
- Prefer the editor diagnostics for quick feedback on files you touched.
...
- Never use `document.querySelector` or `document.querySelectorAll` directly. Always use the `$` and `$$` utilities from a `dom-selector` module instead.